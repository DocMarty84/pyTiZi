...
def Read_TINKER_arc_File(name, mol, verb=2):
	atomic_numbers = input_reading_various.Read_Atomic_Numbers()
	atomic_masses = input_reading_various.Read_Atomic_Masses()
	atomic_valences = input_reading_various.Read_Atomic_Valences()
	try:
		finput = open(name, 'r')
	except:
		if verb > 0:
